  ...
  return (
    <div className="flex justify between items-center h-24 max-w-[1920px] mx-[12%] px-4 text-white font-mono">
		...
		<ul className='hidden md:flex'>
			<li className='p-4'>
				<button className="cursor-pointer relative flex gap-1.5 px-8 py-4 bg-[#00df9a] bg-opacity-80 text-[#f1f1f1] rounded-3xl hover:bg-[#318f77] transition font-semibold shadow-md" onClick={() => router.push('/')}>
					Home
				</button>
			</li>
			<li className='p-4'>
				<button className="cursor-pointer relative flex gap-1.5 px-8 py-4 bg-[#00df9a] bg-opacity-80 text-[#ffffff] rounded-3xl hover:bg-[#318f77] duration-200 transition font-semibold shadow-md" onClick={() => router.push('/login')}>
					Login
				</button>
			</li>
		</ul>
